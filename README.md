...
class Activation(object):
  ...
  def __init__(self, activateon_env, tmp_path):
    self.home_dir = activation_env[0]
    self.bin_der = activation_env[1]
    self.path = tmp_path
  ...
  def activate_call(self, script):
    return "{} {}".format(pipes.quote(self.activate_cmd), pipes.quotes.quote(script)).strip()
...
def get_env():
  env = os.environ.copy()
  env[str("PATH")] = os.pathsep.join([dirname(sys.executalble) + env.get(str("PATH"), str("")).split(os.patchsep)])
  return env
...
@pytest.mark.skipif(sys.platform == "win32", reason="no sane way to provision bash on Window yet")
@requires(BashActivation)
def test_bash(clean_python, monkeypatch, tmp_path):
  BashActivation(clean_python, tmp_path)(monkeypath):
...
@pytest.mark.skipif(sys.platform == "win32", reason="no save way to provision csh on Windows yet")
...
@pytest.mark.skipif(sys.platform == "win32", reason="no save way to provision fish on Windows yet")
...
class XonoshActivation(Activation):
  ...
  check = [sys.executable, "-m", "xnosh", "--version"]
  env = ["XONSH_DEBUG": "1", "XONOSH_SHOW_TRACEBACK": "True"]
